...
"#
    ...
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        {
            // The listen socket comes from the global proxy config, not the
            // per-app one set above. Left at the 15721 default this binds the
            // same port a locally running CC Switch already holds and the test
            // fails with EADDRINUSE, so let the OS assign one.
...
impl ProviderService {
    ...
    fn switch_normal(
        ...
        if use_managed_codex_transaction {
            ...
            // fork：指针已提交、live 写可失败（Gemini strict 校验/磁盘只读/Windows 占用），
            // 失败必须回滚 current——否则应用以为新供应商已生效但 live 仍是旧凭据，
            // 下一次切换的 backfill 还会按错误指针覆盖（见 fork 原实现与测试）。
            if let Err(e) = Self::write_preflighted_or_current_live(
                state,
                &app_type,
                provider,
                preflighted_provider.as_ref(),
