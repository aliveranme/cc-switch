...
fn insert_session_log_entry_on_conn(
    ...
    // 交给下面的 upsert 把它向前推进。注意这里不走 [`should_skip_session_insert`]：
    // 它对 gemini/codex/opencode 的"request_id 存在即跳过"语义会把中间态永久冻结。
...
mod tests {
    ...
    fn test_claude_session_upgrades_intermediate_row_to_final_tokens() -> Result<(), AppError> {
        ...
        let first = {
            let conn = lock_conn!(db.conn);
            insert_session_log_entry_on_conn(&conn, "session:msg_1", &mid)?
        };
        ...
        let second = {
            let conn = lock_conn!(db.conn);
            insert_session_log_entry_on_conn(&conn, "session:msg_1", &done)?
        };
    ...
    fn test_claude_session_upsert_never_moves_tokens_backward() -> Result<(), AppError> {
        ...
        {
            let conn = lock_conn!(db.conn);
            insert_session_log_entry_on_conn(&conn, "session:msg_2", &base)?;
        };
        ...
        {
            let conn = lock_conn!(db.conn);
            insert_session_log_entry_on_conn(&conn, "session:msg_2", &stale)?;
        };
