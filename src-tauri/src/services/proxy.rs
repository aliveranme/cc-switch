...
impl ProxyService {
    ...
    fn write_codex_live_verbatim_with_auth_guard(
        ...
        let prepared_cfg_result = config_str
            ...
            .transpose()
            .map_err(|e| format!("写入 Codex 配置失败: {e}"))
            // fork 4.6：写盘前迁移存量 wire_api = "chat"（上游 Codex 已移除 Chat
            // wire API，遇 "chat" 反序列化直接报错；旧版 CC Switch 模板可能残留该值）
            .map(|prepared| {
                prepared.map(|text| crate::codex_config::migrate_codex_wire_api_in_toml(&text))
            });
