...
- **DeepSeek V4 WebSearch Failed With `Thinking mode does not support this tool_choice`**: `deepseek-v4-pro` / `deepseek-v4-flash` keep thinking mode enabled by default on the OpenAI-compatible endpoint, where only `tool_choice: "auto"` and `"none"` are accepted — `"required"` and the named `{"type":"function","function":{"name":...}}` form both return HTTP 400. Claude Code's built-in WebSearch forces a named tool choice, so every search request through the `openai_chat` conversion 400'd before reaching the model. The proxy now recognises DeepSeek reasoner requests (by model family, or by base URL plus explicit thinking) and downgrades the forced choice to `"auto"`, keeping the request valid while letting the model call the search tool on its own — the same strategy LiteLLM adopted (BerriAI/litellm#27628). The Codex → Chat path gets the same guard. Thinking stays on; only forced/required selection is relaxed, and non-DeepSeek providers are untouched. (<https://api-docs.deepseek.com/guides/thinking_mode/>)
- **Hermes Used AGENTS.md Instead of SOUL.md**: The Hermes prompt file is now written to `SOUL.md`, matching the agent's actual prompt resolution (upstream #5779).
