...
<p align="center">
  <img src="./assets/readme/how-it-works.svg" width="100%" alt="CC Switch の仕組み：単一の SQLite データベースを唯一の情報源とし、切り替え時に各ツールのライブ設定ファイルへアトミックに書き込み、ライブ設定への編集はデータベースへ反映されます">
</p>

- **すべてのツールを 1 つのデータベースで** -- プロバイダ、MCP、Prompts、Skills、使用量データはすべて 1 つの SQLite ファイル（`~/.cc-switch/cc-switch.db`）に保存されます
- **切り替えと同時にライブ設定へ書き込み** -- CC Switch は現在のプロバイダを各ツール固有の設定ファイルにマージし、一時ファイル + リネーム方式で書き込むため、切り替えが中断されても書きかけのファイルが残ることはありません
- **手動編集は失われません** -- 現在のプロバイダのライブ設定ファイルを編集すると、その変更は上書きされずデータベースへバックフィルされます

## 特長

[完全な更新履歴](CHANGELOG.md) | [リリースノート](docs/release-notes/v3.18.0-ja.md)
...
- **便利ツール内蔵** -- 初回起動時のログイン確認、署名バイパス、プラグイン拡張の同期など
- ダーク / ライト / システムテーマ、自動起動、自動アップデーター、アトミック書き込み、自動バックアップ、多言語対応（簡体中文/繁體中文/英/日）
- **Windows、macOS、Linux にネイティブ対応** -- Tauri 2 で構築
